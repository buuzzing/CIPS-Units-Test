...
func main() {
	...
	clog.Info("-------- 14-2-2 长安链-长安链间的跨链非原子写时延 --------")
	fmt.Printf("跨链非原子写时延（完整协议栈）测试统计结果:\n")
	printRecord(crossChainRecord)
	fmt.Printf("\n跨链非原子写时延（未部署协议栈）测试统计结果:\n")
...
func testCrossChain(client *chainmaker_sdk_go.ChainClient, record *[]time.Duration) {
	...
	for {
		select {
		case <-ticker.C:
			clog.Infof("跨链非原子写（完整协议栈）时延测试停止")
			cancel()
			return
		case <-done:
			cost := time.Since(start)
			*record = append(*record, cost)
			clog.Infof("跨链非原子写（完整协议栈）时延测试收到事件，时延: %dms", cost.Milliseconds())
...
func testCrossChainWithoutStack(client *chainmaker_sdk_go.ChainClient, record *[]time.Duration) {
	...
	for {
		select {
		case <-ticker.C:
			clog.Infof("跨链非原子写（未部署协议栈）时延测试停止")
			cancel()
			return
		case <-done:
			cost := time.Since(start)
			*record = append(*record, cost)
			clog.Infof("跨链非原子写（未部署协议栈）时延测试收到事件，时延: %dms", cost.Milliseconds())
	...
	for {
		...
		case event, ok := <-ec:
			...
			if len(contractEvent.EventData) < 2 ||
				contractEvent.EventData[1] != "OpTypeNonAtomicWrite" {
				continue
			}
