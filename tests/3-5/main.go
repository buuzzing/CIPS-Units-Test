...
func init() {
	configFile = flag.String("c", "chainmaker/config/conf3-5.toml", "配置文件路径")
	seq = time.Now().Unix()
}
...
func main() {
	...
	clog.Info("-------- 3-5-1. 向中继链（长安链）发送错误编码的自动请求应答跨链消息 --------")
	test_3_5_1(client)
	clog.Info("-------- 3-5-2. 向中继链（长安链）发送重复的自动请求应答跨链消息 --------")
	test_3_5_2(client)
}

// 中继链（长安链）发送错误编码的自动请求应答跨链消息
func test_3_5_1(client *sdk.ChainClient) {
...
// 向中继链（长安链）发送重复的自动请求应答跨链消息
func test_3_5_2(client *sdk.ChainClient) {
	ccMsg := types.CrosschainMessage{
		SrcChainId:          big.NewInt(1),
		DstChainId:          big.NewInt(2000),
		Seq:                 big.NewInt(seq),
